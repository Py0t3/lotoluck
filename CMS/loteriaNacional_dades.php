<!--
	Página que nos permite mostrar todos los resultados de un sorteo de LAE - Loteria Nacional
	...
	?>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 1);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo, 1);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNacional_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNacional_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(1);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNacional_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
		...
		<script>
			function atras(){
				window.location.href="loteriaNacional.php";
			}
		</script>
	<div id="tabsParaAdicionales"align='left'>
			...
			<div id="adicional_2" class="tabcontent1" style="width:100%;display:none;">
					<div class="adicional_2" align='left' style="margin:10px;">
						...
						<iframe width='100%' height='1000px;' src="puntoVenta3.php?idTipoSorteo=1&idSorteo=<?php echo $idSorteo; ?>"></iframe>
			...
			<div id="adicional_1" class="tabcontent1" style="width:100%;display:block;">
				...
				<?php
					...
					if ($idSorteo != -1)
					{	MostrarSorteoLNacional($idSorteo);			}
					else
					{
						echo "<tr>";
						echo "<td> <label class='cms'> Fecha </label> </td>";

						$fecha = ObtenerFechaSiguienteSorteo(1);

						echo "<td> <input class='fecha' id='fecha' name='fecha' type='date' onchange='ComprovarFecha()' value='$fecha'> </td>";
		...
		<div align='left' style="margin-top: 100px;" id="tabla_premios" name="tabla_premios">
			...
			<table id="principal">
				...
				<?php
				if ($idSorteo == -1) {
					// MostrarCategoriaPremioLAE(1, $idSorteo);
			...
			<div align="right">
				...
				<?php 
					...
					if ($idSorteo <> -1)
					{
						// Obtenemos el numero de terminaciones que hay en la BBDD
						$nTerminaciones = ObtenerNTerminaciones($idSorteo);

						if ($nTerminaciones < 20)
						{		echo "<input id='nt' name='nt' style='display: none;' type='text' value='20'>";		}
						else
						{		echo "<input id='nt' name='nt' style='display: none;' type='text' value='$nTerminaciones'>";		}
					}
					else
					{
						echo "<input id='nt' name='nt' style='display: none;' type='text' value='20'>";
				...
				?>
				<!--<button class='botonGuardar' id='btn_terminaciones' name='btn_terminaciones' style='margin-top:20px;'> <p style='font-size: 16px;' onclick="MostrarMasTerminaciones()"> Nuevo Campo </p> </button>-->
			</div>
		...
		<div style="margin-top:20px;">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(1); echo '</textarea>';
				}
			?>	
		...
		<div style="margin-top:20px;">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(1); echo '</textarea>';
				}
			?>
		</div>
	</div>	
		...
		// Selecciona todos los elementos de entrada y select en el documento
				...
				if(idSorteo==-1){
					...
					$.ajax(
					{
						...
						success: function(res)
						{
							if (res != -1)
							{
								alert("Existe un sorteo del Lotería Nacional registrado para esa fecha, por favor, actualiza el sorteo existente o selecciona otra fecha");
			...
			async function GuardarPremio(idSorteo)
			{
				return new Promise(async function (resolve, reject) {
					...
					$("#principal tbody tr").each(function(i) {
						...
						premio.push($('#nLAE').length > 0 ? $('#nLAE').val() : NULL)
						premio.push($('#nfechaLAE').length > 0 ? $('#nfechaLAE').val() : NULL)
						premio.push($('#fecha').length > 0 ? $('#fecha').val() : NULL)
						premio.push($('#serie').length > 0 ? $('#serie').val() : NULL)
						premio.push($('#fraccion').length > 0 ? $('#fraccion').val() : NULL)
						premio.push($('#terminaciones').length > 0 ? $('#terminaciones').val() : NULL)
			...
			function InsertarPremio(array_premio)
			{
				return new Promise(function (resolve, reject) {
					...
					$.ajax({
						// Definimos la url
						url:"../formularios/premios_loterianacional.php?datos=" + datos,
						data: {array_premio : array_premio},
						type:"POST",
						success: function(data)
						{
							let idSorteo = data.replace(/\"/g, '');
							if (parseInt(idSorteo) > 0)
							{	
								$('#r_id').val(parseInt(idSorteo))
								GuardarComentarios();											
								subirFichero();
			...
			function EliminarReintegroTerminaciones(idSorteo)
			{
			...
			function GuardarTerminaciones(nt, terminaciones, terminaciones_premios, idSorteo, fecha)
			{
				...
				// Función que permite guardar las terminaciones del premio
				fecha = document.querySelector('input[type="date"').value;


				var datos = [fecha, nt, '/', terminaciones, '/', terminaciones_premios, '/'];

				$.ajax(
				{
					// Definimos la url
					url:"../formularios/loteriaNacionalTerminaciones.php?datos=" + datos,
					// Indicamos el tipo de petición, como queremos insertar es POST
					type: "POST",
					...
					success: function(dada)
					{
						if (dada == -1)
						{
							alert("Se ha producido un error al guardar las terminaciones, vuelve a probar. Revisa que se haya introducido todos los valores.");
						}
					}
			...
			function InsertarCategoria()
			{
				...
				if ((descripcion != '') && (descripcion == 'Primer premio' || descripcion == 'Segundo premio' || descripcion == 'Tercer premio' || descripcion == 'Cuarto premio' || descripcion == 'Quinto premio' || descripcion == 'Reintegros' || descripcion == 'terminación' || descripcion == 'Terminación' || descripcion == 'Premio Especial' || descripcion == 'Premio especial'))
				{
					if (posicion != '')
					{
						i= 0;
			...
			function MostrarMasTerminaciones()
			//Se ha modificado respecto a la funcion original de Raquel para que solo añada una fila mas
			
			{
				var nTerminaciones = document.getElementById("nt").value;
				var nombre;

				if (nTerminaciones < 50)
				{
					nTerminaciones = parseInt(nTerminaciones) + 1;
					/*for (i=0;i<10;i++)
					{
						if (nTerminaciones<51)
						{*/
							nombre="titol_t_" + nTerminaciones;
							var input = document.getElementById(nombre);
							input.style.display='block';

							nombre="t_" + nTerminaciones;
							var input = document.getElementById(nombre);
							input.style.display='block';

							nombre="premio_t_" + nTerminaciones;
							var input = document.getElementById(nombre);
							input.style.display='block';

							nombre="euro_t_" + nTerminaciones;
							var input = document.getElementById(nombre);
							input.style.display='block';

							nombre="posicion_t_" + nTerminaciones;
							var input = document.getElementById(nombre);
							input.style.display='block';
							
							nombre="botonEliminar" + nTerminaciones;
							var boton = document.getElementById(nombre);
							boton.style.display='block';

							nTerminaciones = nTerminaciones+1;
						//}
					//}

					$("#nt").val(nTerminaciones-1);
	
				}
				/*
				if (nTerminaciones > 50)
				{
					// Hemos de ocultar el boton porque ya no hay mas posibilidades de insertar terminaciones
					var btn = document.getElementById("btn_terminaciones");
					btn.style.display = 'none';
				}*/
			}
			
			
			function EliminarTerminacion(fila)
			...
			function ActualizarCategoria(idCategoria, nombre, descripcion, posicion)
			{
				...
				var datos = [2, 1, idCategoria, nombre, descripcion, posicion];
