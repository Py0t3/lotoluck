<!--
	Página que nos permite mostrar todos los resultados de un sorteo de LAE - extraordinario
...
<html>
	...
	?>
		...
		<div class='titulo'>
			<table width='100%'>
				<tr>
					<td class='titulo'> Extraordinario - Resultados </td>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 13);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo, 13);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='extraordinario_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='extraordinario_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(13);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='extraordinario_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
		...
		<script>
			function atras(){
				window.location.href="extraordinario.php";
			}
		</script>
	<div id="tabsParaAdicionales"align='left'>
			...
			<table>
				<?php
					...
					if ($idSorteo != -1)
					{	MostrarSorteoExtraordinario($idSorteo);			}
					else
					{
						echo "<tr>";
						...
						echo "<td> <input class='resultados numAnDSer' data-add='principal' id='1premio' name='1premio' type='text' style='text-align:right; width:160px;' value=''> </td>";
						echo "</td>";
						echo "<td> <label class='cms'> Serie: </label> </td>";
						echo "<td>";
						echo "<input class='resultados numAnDSer' data-add='principal' id='serie' name='serie' type='text' value='' style='width: 160px;'>";
						echo "</td>";
						echo "<td>";
		...
		<div align='left' style="margin-top: 100px;">
			...
			<table id ="principal">
				...
				<tbody>
					<?php
					if($idSorteo != -1) {
						MostrarPremiosExtraordinario($idSorteo);
					} else {
						MostrarCategoriasExtraordinario();
					}?>
				</tbody>
			</table>
		...
		<div style="margin-top:20px; margin-left:50px">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(13); echo '</textarea>';
				}
			?>	
		...
		<div style="margin-top:20px; margin-left:50px">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(13); echo '</textarea>';
				}
			?>
		</div>
	</div>
		...
		<script type="text/javascript">
				...
				if (c1 != '' && isNaN(c1)==false)
				{
					...
					if (data != '') {
						...
						if (id=='')
						{
							...
							{
								// Definimos la url
								url:"../formularios/extraordinario.php?datos=" + datos,
								...
								{	
									...
									else
									{
										...
										GuardarPremio(idSorteo,13)
											.then(() => subirFichero())
											.then(() => GuardarComentarios())
											.then(() => {
												window.location.href = "extraordinario_dades.php?idSorteo=" + idSorteo + "&sorteoNuevo=1;";
											})
											.catch(error => {
												console.error(error);
												// Manejo de errores
			...
			$(document).on('change','.numAnDSer',function(e){
			  ...
			  $('#'+tableToApply+' input').filter(function() {
				return normalizeString($(this).val()).indexOf("a las 4 ultimas cifras") !== -1;
			  }).closest('tr').find('.numeros').val('_' + number.substr(number.length - 4));

			  $('#'+tableToApply+' input').filter(function() {
				return normalizeString($(this).val()).indexOf("a las 3 ultimas cifras") !== -1;
			  }).closest('tr').find('.numeros').val('__' + number.substr(number.length - 3));

			  $('#'+tableToApply+' input').filter(function() {
				return normalizeString($(this).val()).indexOf("a las 2 ultimas cifras") !== -1;
			  }).closest('tr').find('.numeros').val('___' + number.substr(number.length - 2));

			  $('#'+tableToApply+' input').filter(function() {
				return normalizeString($(this).val()).indexOf("a la ultima cifra") !== -1;
			  }).closest('tr').find('.numeros').val('____'+ number.substr(number.length - 1));
					...
					if (posicion != '')
					{
						...
						$("#principal tbody tr").each(function(i) {
							...
							if (posicion > $("#principal tbody tr").length) {
								...
								'<td><input class="resultados descripcion" name="nombre" type="text" style="width:400px;" value="'+descripcion+'" onchange="Reset()"></td>'+
								'<td><input class="resultados euros" name="euros" type="text" style="width:150px; text-align:right;" value="0" onchange="Reset()"></td>'+
								'<td class="euro"> € </td>'+
