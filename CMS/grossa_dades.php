	...
	?>
		...
		$(document).ready(function() {
		  ...
		  $(document).on("input", function() {
			...
			$(document).on('keyup','input.acertantes',function(event){				
				$(this).val(function(index, value) {
					return value.replace(/\D/g, "")
					.replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ".");
				});
						...
						if (posicion != '')
						{
							...
							$("#principal tbody tr").each(function(i) {
								...
								if (posicion > $("#principal tbody tr").length) {
									...
									'<td><input class="resultados acertantes" name="acertantes" type="text" style="width:200px;" value="0" onchange="Reset()"></td>'+
									'<td><input class="resultados euros" name="euros" type="text" style="width:150px; text-align:right;" value="0,00" onchange="Reset()"></td>'+
									'<td class="euro"> € </td>'+
									'<td width="50px"> </td>'+
