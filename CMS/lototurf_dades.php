...
<html>
	...
	<main>
		<div class='titulo'> 
				...
				<table width='100%'> 
					<tr>
						<td class='titulo'> LOTOTURF - Resultados </td>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 10);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo, 10);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='lototurf_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='lototurf_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(10);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='lototurf_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
		...
		<script>
			function atras(){
				window.location.href="lototurf.php";
			}
		</script>
				...
				<div class="adicional_1" align='left' style="margin:10px;">
					...
					<div style='margin-left:50px'>
						<table>
							...
							<?php
								if ($idSorteo != -1)
								{	MostrarSorteoLototurf($idSorteo);	}
								else
								{	
									echo "<tr>";
									...
									echo"<td width='50px'></td>";
									echo "<td><input class='resultados' id='caballo' name='caballo' type='text' style='margin:10px;' onchange='Reset()'></td>";
					...
					<div align='left' style="margin-top: 100px;">
						...
						<table id="principal">
							...
							<?php
							if($idSorteo != -1) {
								MostrarPremiosLototurf($idSorteo);
							} else {
								MostrarCategoriasLototurf();
							}	
							?>
							</tbody>
						...
						<table id="tabla_nuevaCategoria" name="tabla_nuevaCategoria" style="display: none; margin-top:30px;">
							<tr>
								<td> <label class="cms" style="width: 150px;"> Categoria </label> </td>
								<td> <label class="cms" style="width: 400px;"> Aciertos </label> </td>
								<td> <label class="cms" style="width: 150px;"> Posición </label> </td>
							</tr>

							<tr>
								<td> <input id="nc_nombre" name="nc_nombre" class="resultados" style="width:150px;"> </td>
								<td> <input id="nc_descripcion" name="nc_descripcion" class="resultados" style="width: 400px;"> </td>
								<td> <input id="nc_posicion" name="nc_posicion" class="resultados" style="width: 150px;"> </td>
							</tr>
					...
					<div style="margin-top:20px;">
						...
						<?php
							if ($idSorteo <> -1) {
								...
								echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(10); echo '</textarea>';
							}
						?>	
					...
					<div style="margin-top:20px;">
						...
						<?php
							if ($idSorteo <> -1) {
								...
								echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(10); echo '</textarea>';
							}
						?>
					</div>
				</div>
									...
									if (c6 != '' && isNaN(c6)==false)
 									{
										if (isNaN(caballo)==false)
										{
											if (isNaN(reintegro)==false)
											{
												if (data != '')
												{
													...
													{
														// Se ha de insertar															
														var datos = [1, -1, c1, c2, c3, c4, c5, c6, caballo, reintegro, data];
														console.log(datos)
														$.ajax(
														{
															// Definimos la url
															url:"../formularios/lototurf.php?datos=" + datos,
															// Indicamos el tipo de petición, como queremos insertar es POST
															type:"POST",	
															...
															{	
																...
																else
																{
																	...
																	GuardarPremio(idSorteo,12)
																		...
																		.then(() => {
																			window.location.href = "lototurf_dades.php?idSorteo=" + idSorteo + "&sorteoNuevo=1;";
																		})
																		.catch(error => {
																			console.error(error);
																			// Manejo de errores
													...
													{
														// Se ha de actualizar
														var datos = [2, id, c1, c2, c3, c4, c5, c6, caballo, reintegro, data];
														console.log(datos)
														$.ajax(
														{
															// Definimos la url
										...
										{	alert("No se puede guardar el juego porque el caballo no es númerico");		}
									}
 									else
		...
		function InsertarPremio(array_premio,idJuego)
		{
			 return new Promise((resolve, reject) => {
				...
				var	urlForm='premios_lototurf.php';
