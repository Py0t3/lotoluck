...
<html>
	...
	?>
		...
		<div class='titulo'>
			...
			<table width='100%'>
				<tr>
					<td class='titulo'> Lotería Navidad - Resultados </td>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 2);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo, 2);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNavidad_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNavidad_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(2);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='loteriaNavidad_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
	...
	<div id="tabsParaAdicionales"align='left'>
			...
			<div id="adicional_1" class="tabcontent1" style="width:100%;display:block;">
				...
				<?php
					...
					if ($idSorteo != -1)
					{	MostrarSorteoLNavidad($idSorteo);			}
					else
					{
						echo "<tr>";
						echo "<td> <label class='cms'> Fecha </label> </td>";

						$fecha = ObtenerFechaSiguienteSorteo(2);
						...
						echo "<td> <input class='resultados' id='nLAE' name='nLAE' type='text' style='width:165px;display:none;' value='' > </td>";
		...
		<div align='left' style="margin-top: 100px;" id="tabla_premios" name="tabla_premios">
			...
			<table id ="principal">
				...
				<?php
				if($idSorteo == -1) {
					PremiosParaNuevoRegistroLoteriaNavidad();
				} else {
					MostrarPremiosLoteriaNavidad($idSorteo);
		...
		<div style="margin-top:20px;">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(2); echo '</textarea>';
				}
			?>	
		...
		<div style="margin-top:20px;">
			...
			<?php
				if ($idSorteo <> -1) {
					...
					echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(2); echo '</textarea>';
				}
			?>
		</div>
		</div>
		...
		<script type="text/javascript">
			...
			async function Guardar()
			{
				...
				if (fecha != '')
				{
					...
					EliminarCuartoQuintos(idSorteo);
			...
			function InsertarPremio(array_premio) {
					return new Promise(function (resolve, reject) {
						...
						$.ajax({
							// Definimos la url
							url: "../formularios/premios_loterianavidad.php?datos=" + datos,
			...
			function InsertarCategoria()
			{
				...
				if ((descripcion != '') && (descripcion == 'Primer premio' || descripcion == 'Segundo premio' || descripcion == 'Tercer premio' || descripcion == 'Cuarto premio' || descripcion == 'Quinto premio' || descripcion == 'Reintegro' || descripcion == 'Premio especial'))
				{
					if (posicion != '')
					{
						i= 0;
			...
			function ActualizarCategoria(idCategoria, nombre, descripcion, posicion)
			{
				...
				var datos = [2, 2, idCategoria, nombre, descripcion, posicion];
