...
<html>
    ...
    <script>
     // Importa las bibliotecas necesarias de jQuery y TinyMCE
// Asegúrate de cargar las bibliotecas correctamente en tu proyecto
// Función para cargar la lista de imágenes disponibles desde el servidor
function cargarListaDeImagenes() {
  fetch('https://lotoluck.es/imagenesTinyMCE.php') // Reemplaza con la URL de tu archivo PHP
    .then(response => response.json())
    .then(data => {
      // Llama a TinyMCE y personaliza la ventana de diálogo de imágenes
      tinymce.init({
        selector: 'textarea',
        plugins: 'image',
        toolbar: 'image',
        ...
        file_picker_callback: function (cb, value, meta) {
          // Abre un cuadro de diálogo personalizado para seleccionar imágenes
          const customImageDialog = document.createElement('div');
          customImageDialog.innerHTML = '<h2>Selecciona una imagen</h2>';
          
          // Recorre la lista de imágenes y muestra cada una como opción
          data.forEach(imageUrl => {
            ...
            imageOption.addEventListener('click', function () {
              // Cuando el usuario hace clic en una imagen, inserta la URL de la imagen en el editor
              cb(imageUrl);
              customImageDialog.remove();
            });
            customImageDialog.appendChild(imageOption);
          });
          
          document.body.appendChild(customImageDialog);
        }
      });
    })
    .catch(error => console.error('Error al cargar la lista de imágenes:', error));
}

// Llama a la función para cargar la lista de imágenes cuando se carga la página
window.addEventListener('DOMContentLoaded', cargarListaDeImagenes);
