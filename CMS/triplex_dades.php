...
<html>
	...
	?>
		...
		<div class='titulo'> 
				...
				<table width='100%'> 
					<tr>
						<td class='titulo'> TRIPLEX - Resultados </td>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 18);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo, 18);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='triplex_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='triplex_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(18);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='triplex_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
		...
		<script>
			function atras(){
				window.location.href="triplex.php";
			}
		</script>
				...
				<div style='margin-left:50px'>
					<table>
						...
						{	
							...
							echo "<td><input class='resultados' id='nSorteo' name='nSorteo' type='text' style='margin:10px;' value='' onchange='Reset()'></td>";
							echo "<td width='50px'></td>";
							echo "<td> <input class='resultados numAnDSer' data-add='principal' id='r_c1' name='r_c1' type='text' style='margin:10px;' value=''>";
							echo "<input class='resultados numAnDSer' data-add='principal' id='r_c2' name='r_c2' type='text' style='margin:10px;' value=''>";
							echo "<input class='resultados numAnDSer' data-add='principal' id='r_c3' name='r_c3' type='text' style='margin:10px;' value=''></td>";
							echo"<td width='50px'></td>";
							echo "</tr>";
							echo "<tr>";
				...
				<div align='left' style="margin-top: 100px;">
					...
					<table id="principal">
						...
						<tbody>
							<?php
							if($idSorteo != -1) {
								MostrarPremiosTriplex($idSorteo);
							} else {
								MostrarCategoriasTriplex();
							}	
							?>
						</tbody>
					</table>
				...
				<div style="margin-top:20px; margin-left:50px">
					...
					<?php
						if ($idSorteo <> -1) {
							...
							echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(18); echo '</textarea>';
						}
					?>	
				...
				<div style="margin-top:20px; margin-left:50px">
					...
					<?php
						if ($idSorteo <> -1) {
							...
							echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(18); echo '</textarea>';
						}
					?>
				</div>
			</div>	
		...
		<script type="text/javascript">
            ...
            $('#fecha').change(function(){
                ...
                console.log('cambio')
                var datos = [5, -1, c1, c2, c3, nSorteo, data];
				$.ajax({
				    // Definimos la url
					url:"../formularios/triplex.php?datos=" + datos,
 						...
 						if (c3 != '' && isNaN(c3)==false) {
                            if (nSorteo != '' && isNaN(nSorteo)==false) {
                                if (data != '') {
                                    ...
                                    {
                                        // Se ha de insertar															
                                        var datos = [1, -1, c1, c2, c3, nSorteo, data];
                                        ...
                                        $.ajax(
                                        {
                                            ...
                                            {	
                                                ...
                                                else
												{
													...
													GuardarPremio(idSorteo,18)
														.then(() => subirFichero())
														.then(() => GuardarComentarios())
														.then(() => {
															window.location.href = "triplex_dades.php?idSorteo=" + idSorteo + "&sorteoNuevo=1;";
														})
														.catch(error => {
															console.error(error);
															// Manejo de errores
                                    ...
                                    {
                                        // Se ha de actualizar
                                        var datos = [2, id, c1, c2, c3, nSorteo, data];
			...
			// 			case 78:
			// 				if (r_c1 != '' && r_c2 != '' && r_c3 != '') {
			// 					let numberCombination = permutator([r_c1,r_c2,r_c3])
			// 					numberCombination.shift()
			// 					$('#numeros_78').val(numberCombination.join(' | '))
			// 				}
			// 				break;
			// 			case 79:
			// 				$('#numeros_79').val(r_c1+''+r_c2+'_')
			// 				break;
			// 			case 80:
			// 				$('#numeros_80').val(('_'+r_c2+''+r_c3))
			// 				break;
			// 			case 81:
			// 				$('#numeros_81').val((r_c1+'__'))
			// 				break;
			// 			case 82:
			// 				$('#numeros_82').val(('__'+r_c3))
			// 				break;
			// 			default:
			// 				break;
			// 		}
					...
					if (posicion != '')
					{
						...
						$("#principal tbody tr").each(function(i) {
							...
							if (posicion > $("#principal tbody tr").length) {
								...
								'<td><input class="resultados numeros" name="numero" type="text" style="width:320px; text-align:right;" value=""></td>'+
