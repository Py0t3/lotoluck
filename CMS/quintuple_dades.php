...
<!DOCTYPE html>
	...
	?>
		...
		<div class='titulo'> 
				...
				<table width='100%'> 
					<tr>
						<td class='titulo'> QUINTUPLE - Resultados </td>
		...
		<div style='text-align: right;'>
			<?php
				...
				$idSorteoPosterior = ExisteSorteoPosterior($idSorteo, 11);
				$idSorteoAnterior = ExisteSorteoAnterior($idSorteo,11);
				
				if ($idSorteoAnterior != -1)
				{
					echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='quintuple_dades.php?idSorteo=$idSorteoAnterior'> Anterior </a> </button>";
				}
				if ($idSorteoPosterior != -1|| isset($_GET['sorteoNuevo']))
				{
				
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='quintuple_dades.php?idSorteo=$idSorteoPosterior'> Siguiente </a> </button>";

					
				}
				if($idSorteo==-1 ){
						$primerSorteo= devolverPrimerSorteo(11);
						echo "<button class='botonSiguiente' style='margin-right:1em;padding:0.6em;'> <a class='cms_resultados' href='quintuple_dades.php?idSorteo=$primerSorteo'> Siguiente </a> </button>";
		...
		<script>
			function atras(){
				window.location.href="quintuple.php";
			}
		</script>
			...
			<div id="adicional_1" class="tabcontent1" style="width:100%;display:block;">
				<div class="adicional_1" align='left' style="margin:10px;">
					...
					<div style='margin-left:50px'>
						...
						<?php
						if ($idSorteo != -1)
						{	MostrarSorteoQuintuple($idSorteo);	}
						else
						{	
							echo "<tr>";
							...
							echo "<input class='resultados' id='r_c6' name='r_c6' type='text' style='margin:10px;'> </td>";
							echo"<td width='50px'></td>";
					...
					<div align='left' style="margin-top: 100px;">
						...
						<table id="principal">
							...
							<?php
							if($idSorteo != -1) {
								MostrarPremiosQuintuple($idSorteo);
							} else {
								MostrarCategoriasQuintuple();
							}	
							?>
							</tbody>
						</table>
					...
					<div style="margin-top:20px;">
						...
						<?php
							if ($idSorteo <> -1) {
								...
								echo '<textarea name="textoBanner" rows="10" cols="90" id="textoBanner" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_txtBanner(11); ;echo '</textarea>';
							}
						?>	
					...
					<div style="margin-top:20px;">
						...
						<?php
							if ($idSorteo <> -1) {
								...
								echo '<textarea name="comentario" rows="10" cols="90" id="comentario" style="margin-top: 6px; width:600px;">';echo obtener_ultimo_comentario(11); ;echo '</textarea>';
							}
						?>
					</div>
				</div>
		...
		<script type="text/javascript">
									...
									if (c6 != '' && isNaN(c6)==false)
 									{
                                        if (data != '')
                                        {
                                            ...
                                            {
                                                // Se ha de insertar															
                                                var datos = [1, -1, c1, c2, c3, c4, c5, c6, data];
                                                console.log(datos)
                                                $.ajax(
                                                {
                                                    // Definimos la url
                                                    url:"../formularios/quintuple.php?datos=" + datos,
                                                    // Indicamos el tipo de petición, como queremos insertar es POST
                                                    type:"POST",	
                                                    ...
                                                    {	
                                                        ...
                                                        else
														{
															...
															GuardarPremio(idSorteo,12)
																...
																.then(() => {
																	window.location.href = "quintuple_dades.php?idSorteo=" + idSorteo + "&sorteoNuevo=1;";
																})
																.catch(error => {
																	console.error(error);
																	// Manejo de errores
                                            ...
                                            {
                                                // Se ha de actualizar
                                                var datos = [2, id, c1, c2, c3, c4, c5, c6, data];
                                                console.log(datos)
                                                $.ajax(
                                                {
                                                    // Definimos la url
			...
			function InsertarPremio(array_premio,idJuego)
			{
				 return new Promise((resolve, reject) => {
					...
					var	urlForm='premios_quintuple.php';
			...
			function ActualizarCategoria(idCategoria, nombre, descripcion, posicion)
			{
				...
				var datos = [2, 11, idCategoria, nombre, String(descripcion).replaceAll('+', '%2B'), posicion];
				console.log(datos)
				// Realizamos la petición ajax
				$.ajax(
				{
